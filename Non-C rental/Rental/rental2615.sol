...
import "../ERCX/Contract/ERCXHolder.sol";

import "./IPayment.sol";
import "./IvRent2615.sol";

contract vRent is IvRent2615, ERC721Holder, ERCXHolder {
    ...
    function bundleCall(function(CallData memory) _manager, CallData memory _cd)
        private
    {
        require(_cd.tokenIds.length > 0, "vRent::no nfts");
        _manager(_cd);
    }
    ...
    function rentNFT(
        ...
        bundleCall(
            manageRent,
            _createRentCallData(_tokenIds, _leasingIds, _rentDurations)
        );
    }
     ...
     */
    function endRent(uint256[] memory _tokenIds, uint256[] memory _leasingIds)
        external
        override
        notPaused
    {
        bundleCall(manageReturn, _createActionCallData(_tokenIds, _leasingIds));
    }
    ...
    function cancelLeasing(
        ...
        bundleCall(
            manageStopLeasing,
            _createActionCallData(_tokenIds, _leasingIds)
        );
    }
    ...
    function manageLease(CallData memory _cd) private {
        for (uint256 i = 0; i < _cd.tokenIds.length; i++) {
            _verifyIsLeaseable(_cd, i);

            LeasingRenting storage item = leasingRenting[
                keccak256(abi.encodePacked(_cd.tokenIds[i], leasingId))
            ];
            ...
            // approve tenant right to vera contract
            _ERCX.approveTenantRight(address(this), _cd.tokenIds[i]);
            // transfer user ownership to vera contract
            _ERCX.safeTransferUser(
                item.Leasing.leaserAddress,
                address(this),
                _cd.tokenIds[i]
            );
            leasingId++;
    ...
    function manageRent(CallData memory _cd) private {
        for (uint256 i = 0; i < _cd.tokenIds.length; i++) {
            LeasingRenting storage item = leasingRenting[
                keccak256(abi.encodePacked(_cd.tokenIds[i], _cd.leasingIds[i]))
            ];
            ...
            // transfer user ownership to user
            _ERCX.safeTransferUser(address(this), msg.sender, _cd.tokenIds[i]);
            // set tenant rights to vera contract
            _ERCX.setTenantRight(_cd.tokenIds[i]);
        }
    }
    ...
    function manageReturn(CallData memory _cd) private {
        for (uint256 i = 0; i < _cd.tokenIds.length; i++) {
            ...
            _ERCX.revokeTenantRight(_cd.tokenIds[i]);
            _ERCX.safeTransferUser(msg.sender, address(this), _cd.tokenIds[i]);
            _ERCX.approveTenantRight(address(this), _cd.tokenIds[i]);
    ...
    function manageStopLeasing(CallData memory _cd) private {
        for (uint256 i = 0; i < _cd.tokenIds.length; i++) {
            ...
            // _ERCX.revokeTenantRight(_cd.leasingIds[i]);
            _ERCX.safeTransferUser(address(this), msg.sender, _cd.tokenIds[i]);
            delete item.Leasing;
        }
    }
